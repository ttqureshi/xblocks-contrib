...
class DiscussionXBlock(XBlock, StudioEditableXBlockMixin, XmlMixin):  # lint-amnesty, pylint: disable=abstract-method
    ...
    def has_permission(self, permission):
        ...
        # :rtype: bool
        # """
        assert isinstance(self.course_key, (type(None), CourseKey))
        ...
        cache_key = "django_comment_client.permissions.has_permission.all_permissions.{}.{}".format(
            self.django_user.id, self.course_key
        )
        if cache_key in request_cache_dict:
            all_permissions = request_cache_dict[cache_key]
        else:
            all_permissions = self.all_permissions_for_user_in_course(self.django_user, self.course_key)
